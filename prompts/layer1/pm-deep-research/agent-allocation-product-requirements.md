...
> **段3 Cagan 4-risks = 4 micro-aspect**：段3 以 4 个 single-class micro-aspect 落地（`cagan-risk-value` / `-usability` / `-feasibility` / `-business`），每个只评 1 类风险、`max_search_calls=4`、bounded 预算强制收敛。**Why**：让单个 aspect 同时覆盖 4 类风险，在 strategist persona 下容易陷入持续搜证不收敛；改为 4 个 bounded single-class 任务，每个职责单一、预算受限，自然收敛，并能保留 dedicated 段3 的输出深度。段3 hard gate "4 类全覆盖" = "4 个 micro-aspect 全 present 且各自该类完备"；final-report Phase B 段3 从 4 micro-aspect 装配。段3 仍是 **Strategist 拥有的一个段**（段所有权不变）。

### EA + Strategist balanced persona ownership note

One Lapis aspect = one persona，所以方法论中 "EA + Strategist 双签 / 共同" 的段在 Lapis 接口下必须收敛到单一 persona 主笔：
...
本 profile 设 **5 个 hard gate 段**（4 profile 中最严，PRD 前置物是 build-input，缺一项下游开发就会卡）：

| 段 | hard gate | 触发缺口 | 处理 |
|---|---|---|---|
| 3 | **Cagan 4-risks 全覆盖**（4 micro-aspect 全 present） | 缺任一 risk-class micro-aspect 或其类未完备 | 缺失 micro-aspect 触发 Phase A backfill；仍缺即整段 fail |
| 4 | **OST ≥3 候选 / outcome** | <3 候选 | 整段 fail + Phase A backfill |
| 5 | **非目标 显式列** | 缺 "不做什么" 段 | 整段 fail + Phase A backfill (PR-FAQ 文化核心) |
| 6 | **三套指标 (主/次/护栏)** | 缺一套 | 整段 fail + Phase A backfill |
| 8 | **TM-11 falsification 100%** | 任一未决问题缺 "靠什么会决" | 整段 fail + Phase A backfill |

> Strategist + EA 双侧人格在各自段内必须 enforce 本段 hard gate，不可外推到 final-report Phase C 兜底 (那是兜底机制，不是首选)。

### 8-section PR-FAQ template "working backwards" PR-FAQ assembly note

PR-FAQ 段1 看似在最前，但 Amazon working backwards 实际流程是：先 JTBD + ODI (段2) → 综合 4-risks + OST (段3+4) → 写需求 + metrics (段5+6) → **回填** PR-FAQ (段1 价值主张语句)。

Lapis 8 段并行 (≤3 concurrent) 跑完后，**final-report Phase B 在 chapter assembly 阶段做 PR-FAQ 回填校验**：段1 strategist 在 aspect-research 中写出的 PR-FAQ structure (headline / FAQ 等) 在 final-report 中必须 verify "价值主张语句是否 trace 回段2 ODI outcomes"。这是 8-section PR-FAQ template 不同于 13-section narrative report 的关键 narrative 流。
...
- `build` (default): 段1 PR-FAQ 强制 "新产品上线日" 风格 (Amazon 经典 PR-FAQ format); 段4 OST 强制 ≥1 "新建 vs 复用既有平台" 对比; 段6 metrics 主指标含激活/留存 leading metric; 段1 `max_search_calls` +1 (PR-FAQ 借鉴 Amazon-style 范例).
...
7. **段3 / 段4 / 段5 / 段6 / 段8 是 hard floor aspects** — 缺对应 hard gate (4-risks 全 / ≥3 候选 / 非目标 / 三套指标 / TM-11) → 整段 fail, 拒绝软化. 4 profile 中 hard-gate density 最高 (5 hard gates), by design.
8. 段1 PR-FAQ 不可包含实现细节 (技术架构 / 模块名 / DB schema 等) — strategist 在 success_criteria 中显式禁止.
9. 段4 OST 候选可借段3 4-risks viability evidence ids 做快评, 减少独立 search 消耗 (与 strategist 借 EA prior 同模式).
