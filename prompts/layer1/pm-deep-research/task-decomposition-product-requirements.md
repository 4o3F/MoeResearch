...
Follow the validated mapping in [`agent-allocation-product-requirements.md`](agent-allocation-product-requirements.md). Summary:
...
- **段7 evidence-table 是 OPTIONAL**: **默认不单独 spin 一个 evidence-table aspect**。理由：Lapis `evidence_refs` 限定在 aspect 自己的 search 输出、**不许 cite prior_sources by id**；evidence-table 本质是 meta-aggregation 任务（汇总 段1-6+8 的跨段证据），让它自己再 search 一遍既浪费 budget 又会 `schema_validation_failed`（validated: multiple retries failed）。**默认 fallback**：4-tier 全套证据表由 [`final-report-product-requirements.md`](final-report-product-requirements.md) Phase B 跨段聚合产出（等价）。**仅当**用户显式要求一份 standalone evidence appendix（`deep_evidence_pack` preset 或显式 "evidence pack" 意图）时才 spin 段7 aspect；此时它的 `success_criteria` 须显式声明 "聚合 prior aspects 的 findings，不重复 search"。deep tier 因此默认 10 mandatory aspect（段1,2 + 段3×4 cagan micro + 段4,5,6,8），`max_agents=11` 仍预留 1 位给可选段7、不破预算包络。
...
- **Deep `max_total_search_calls=64`** = 移除原单 cagan aspect 的 ~8，加 4 micro × 4 = 16.
- **Standard `max_agents=7`** = 段1,2 + 段3×4 cagan micro + 段4；model/search 同比例上调。
- 实测撞 budget → 顺序重试 + prior_sources 喂入 baseline.
...
- **Per-aspect `timeout_ms` 恒 600000 (10 min)** — slow model+search backends; 300000 → `budget_exceeded` (validated).
- **Per-aspect `max_search_calls=6`** — PR-FAQ + OST 候选枚举需多查范例（同 build intent 加成）.
- **段3 cagan micro-aspect 用更小预算（`max_turns=5` / `max_tool_calls=6` / `max_search_calls=4`）**：单 1 类风险任务，bounded 预算**强制收敛**——这是修复 search-saturation 的核心机制（single-class search=4 可收敛）。
...
> Same `DeepResearchRequest` wire shape as competitive / product-capability / innovation-direction — Lapis `schema_version="0.1"` 不变；`search_policy` **不挂** `depth`/`content_level`/`recency`/`category`. **Search-tuning with `recency=fresh` was investigated but NOT adopted for this profile** — `requirements-fn-nfn-nongoals` aspect is structurally synthesis-time fragile under `recency=fresh` (validated: multiple retry patterns all fail). PR-FAQ assembly aspects have higher synthesis density than other profiles; validated golden scores are already at ceiling; adding search-tuning introduces fragility without rubric benefit. Revisit when the engine supports per-aspect `search_policy` override or per-aspect `execution_policy.timeout_ms`. Details: [`product-requirements` golden evaluation](../../../docs/pm-deep-research/evaluation/golden/product-requirements-rubric-score.md).
>
> **`execution_policy.timeout_ms` 必须等于 per-aspect `budget.timeout_ms` (600000)**, NOT `total_timeout_ms` — validated: each aspect is re-checked against this value (`budget_exceeded` if mismatched).
