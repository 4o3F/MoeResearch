...
- **段8 TM-11 hard gate**: 每未决问题必须含 "靠什么会决"（discovery sprint / prototype / A/B test 等可执行实验设计），不可写 "需要更多研究" 此类空话. 缺 falsification → aspect 整段 0 分. 这是 v2.2 段8 TM-11 hard gate 在 8-section PR-FAQ template 模板下的对应延续。

- **段7 evidence-table 是 OPTIONAL（，2026-05-30）**: **默认不单独 spin 一个 evidence-table aspect**。理由：Lapis `evidence_refs` 限定在 aspect 自己的 search 输出、**不许 cite prior_sources by id**；evidence-table 本质是 meta-aggregation 任务（汇总 段1-6+8 的跨段证据），让它自己再 search 一遍既浪费 budget 又会 `schema_validation_failed`（M7 实测 2 次 retry 全失败）。**默认 fallback**：4-tier 全套证据表由 [`final-report-product-requirements.md`](final-report-product-requirements.md) Phase B 跨段聚合产出（等价，M7 实测 59 evidence 聚合成功）。**仅当**用户显式要求一份 standalone evidence appendix（`deep_evidence_pack` preset 或显式 "evidence pack" 意图）时才 spin 段7 aspect；此时它的 `success_criteria` 须显式声明 "聚合 prior aspects 的 findings，不重复 search"。deep tier 因此默认 10 mandatory aspect（段1,2 + 段3×4 cagan micro + 段4,5,6,8），`max_agents=11` 仍预留 1 位给可选段7、不破预算包络。
...
- **Deep `max_agents=11`** = 10 mandatory（段1,2 + 段3×4 cagan micro + 段4,5,6,8）+ 1 预留给可选段7（；原 8）。
- **Deep `max_total_model_calls=80`** = 7 个 full aspect (~8 calls each) + 4 个 cagan micro (~5 calls each) + margin（；原 60）.
- **Deep `max_total_search_calls=64`** = 移除原单 cagan aspect 的 ~8，加 4 micro × 4 = 16（；原 42）.
- **Standard `max_agents=7`** = 段1,2 + 段3×4 cagan micro + 段4（；原 4）；model/search 同比例上调。
...
- **Per-aspect `max_search_calls=6`** 比 v2.2 的 5 多 1 因 8-section PR-FAQ template PR-FAQ + OST 候选枚举需多查范例（同 build intent 加成）.
- **段3 cagan micro-aspect 用更小预算（`max_turns=5` / `max_tool_calls=6` / `max_search_calls=4`）**：单 1 类风险任务，bounded 预算**强制收敛**——这是  修复 search-saturation 的核心机制（M7 单 aspect 4-class 在 search=8+ 下不收敛；single-class search=4 可收敛）。
- **`total_timeout_ms = ceil(max_agents / max_concurrent_agents) × per_aspect_timeout_ms`** — Quick (1 wave) `600000`；Standard (7/3=3 waves) `1800000`；Deep (11/3=4 waves) `2400000`（；原 Standard 1200000 / Deep 1800000）.
