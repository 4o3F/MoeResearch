...
`subject_domain` is required — innovation-direction research is **赛道层级 / 跨现状看未来**, not 单产品纵深 (that's v2.1 product-capability). `target_actor` is optional incumbent observer used for "现状承载力" baseline; may be omitted. `time_window_months` default 24 (12-36 月窗口), profile §2 段1 强制 < `time_window_months`.

If `budget_preset` is null, infer the tier from §2.

## Step 1 — Infer `decision_intent` (mandatory, before any decomposition)

Pick exactly one (spec §1.2; innovation-direction profile §1 表 1 默认集 = `ai-upgrade` / `enter` / `differentiate`):
...
| `improve` / `grow` / `build` | Out of scope for innovation-direction | Re-route: `improve` → product-capability (v2.1); `build` → product-capability 段6 (build-cost only); `grow` → product-requirements (v2.3) |
...
- **段2 sole EA aspect**：未满足 job + outcome (ODI underserved) 是赛道用户视角 — EA 本职. 但本 profile EA 只出场一次, 段4 / 段7 中 "对位" / "我方承载力" 等需要 EA 视角的判断, 由 strategist aspect 通过 `shared_context.prior_sources` 引用段2 EA 输出后 fold-in (与 v2.1 段5 strategist 用 EA 数据同模式).

- **段8 TM-11 hard gate**：`recommended-bets` 的 `success_criteria` 必须显式列：每推荐下注 ≥1 "什么条件下错" (leading indicator + 阈值). 缺 falsifiability → aspect 整段 0 分, 触发 Phase A backfill (final-report 报告器执行). 这是 innovation-direction profile 与 v2.0/v2.1 最大差异.
...
- `scope` / `boundaries`: from the segment's method (profile §2) + subject_domain + 时间窗.
...
- **Deep 8 段**: per-aspect 实测 max_search 6 × 8 aspect = 48，top 50 留 small headroom（R4-c v2.2 验证 cap=6 必须，详见 per-aspect 段）.
...
- **Deep `max_search_calls` is 6 — strictly higher than v2.0 (cap=4) and v2.1 (cap=3) (R4-c v2.2, 2026-06-02)** — 同 v2.0 hard-kill 半语义（`crates/lapis-workflow/src/agent_loop.rs`），但 v2.2 的 `recommended-bets` 是综合下注 aspect（吃前 7 个 aspect 输出做最终下注合成），自然 search appetite ≈ 6；rerun9 anchor cap=5 历史上稳，但加 `recency=fresh` 后 prompt-hint 把 appetite 推到 6 → cap=5 hard-kill 复现（R4-c v2.2 验证：cap=5 retry 2 次均 budget_exceeded，cap=6 一次过）. **不要降到 5 以下、也不要拔到 7+**（cap=6 是验证过的稳态）.
- **Per-aspect `timeout_ms` 恒 600000 (10 min)** — CPA(gpt-5.5)+grok-4.3 慢；300000 → `budget_exceeded` (M4 实测)；偶发 grok 单 search 慢抖动 → watchdog 700s 重试即正常（R4-c v2.2 实测 trend-scan 第一次 transient timeout）.
...
- Search-provider 指引：
  ...
  - 单一 provider 时全用之.
- **Search-tuning (R4-c v2.2, validated 2026-06-02)**：set `search_policy.recency = "fresh"` + `search_policy.max_results_per_query = 5`. **Global** field; ceiling + default + prompt-hint. 与 v2.0/v2.1 的关键区别 = **必须同时把 deep per-aspect `max_search_calls` 抬到 6**——否则 recommended-bets 的 prompt-hint-推动 appetite 撞 cap=5 hard-kill（详 per-aspect budget 段 + [innovation-direction R4-c 评分](../../../docs/pm-deep-research/evaluation/golden/innovation-direction-rubric-score.md)）. **不要**设 `depth=high_recall`（怂恿过搜，与 cap=6 同时启用一起会再次撞墙）/ `content_level=detailed`（mutated_provenance）/ `category`（exact-match 不能全局——innovation 段4 `category=organizations` per-aspect 启用要等引擎支持 per-aspect search_policy，未来 WS-U 后改进）. 详 [interface §5.1/§6.1](../../../docs/pm-deep-research/orchestration-interface.md#51-上游-search-tuning-字段e04398d5--7r4-d-回补).
...
> Same `DeepResearchRequest` wire shape as competitive / product-capability — Lapis `schema_version="0.1"` 不变。`search_policy` **已挂上游 R4-c canonical `recency=fresh` + `max_results_per_query=5`**（R4-c v2.2 2026-06-02 验证：8/8 收敛、24/24 锚点持平、+11 evidence +1 domain）。**与 v2.0/v2.1 关键差异**：v2.2 deep per-aspect cap=6（不是 v2.0=4 / v2.1=3），原因 = recommended-bets 是综合下注 aspect、recency=fresh prompt-hint 把 search appetite 推到 ~6，cap=5 必死。**未挂** `depth`/`content_level`/`category`（同 v2.0/v2.1 理由：hard-kill / mutated_provenance / 不能全局），段4 `category=organizations` per-aspect 启用要等引擎支持 per-aspect search_policy（未来 WS-U 后改进）.
>
