...
> Same `DeepResearchRequest` wire shape as competitive — MoeResearch `schema_version="0.1"` 不变。`search_policy` emits the complete DTO shape: use `recency=fresh` + `max_results_per_query=5`, and set `depth` / `content_level` / `category` to `null` because broad-recall hints can over-search and global category filters can misroute mixed aspects.
>
