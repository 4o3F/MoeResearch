...
> Competitive specialization of the Lapis report-synthesis step. Turns a validated `DeepResearchResult` into the **13-chapter competitive report**, then self-verifies against the quality floor. This is the Skill-layer assembly step. Personas/aspects: [`agent-allocation.md`](agent-allocation.md).
...
Before writing, run this checklist over `aspect_reports` + `evidence_index` + `failed_aspects`. For each gap, either (a) trigger one orchestration backfill round — re-call `aspect_research` for the deficient aspect, passing `shared_context.prior_sources` = already-collected evidence to avoid repeats (**Standard ≤1 round, Deep ≤2 rounds**, then stop — bounded self-refine) — or (b) if backfill is exhausted/impossible, mark it explicitly in Ch 12 and lower the affected confidence. Never silently paper over a gap.
