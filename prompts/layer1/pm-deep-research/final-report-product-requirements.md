...
> Product-requirements specialization of the Lapis report-synthesis step. Turns a validated `DeepResearchResult` into the **8-段 PR-FAQ template** (distinct from the 13-chapter template used by the other three profiles). Self-verifies against the quality floor with **five hard gates** (段3 4-risks 全 / 段4 ≥3 候选 / 段5 非目标 显式 / 段6 三套指标 / 段8 TM-11 falsification 100%). Skill-layer assembly step (interface §1 steps 6–9). Authority: universal frame [spec §7.1 报告模板族](../../../docs/pm-deep-research/pm-deep-research-spec.md#71-报告模板族a13-章--b8-段-pr-faq) + [§7.5 prose floor](../../../docs/pm-deep-research/pm-deep-research-spec.md#75-prose-写作硬底线) + [§9 / §6](../../../docs/pm-deep-research/pm-deep-research-spec.md) + product-requirements profile [§2 八段骨架 / §3 gap & floor](../../../docs/pm-deep-research/capabilities/product-requirements.md). Personas/aspects: [`agent-allocation-product-requirements.md`](agent-allocation-product-requirements.md).
...
### 与 13 章模板 vs 8 段 PR-FAQ 模板写法的关键对照

| 13 章模板 (competitive / product-capability / innovation-direction) | 8 段 PR-FAQ 模板 (product-requirements) | 区别 |
|---|---|---|
| Ch 1 研究结论摘要 (BLUF/SCQA) | **段 1 PR-FAQ 本身即是 BLUF** | 8 段模板不另起 BLUF |
| Ch 4 用户人群与 JTBD | 段 2 机会验证 (含 JTBD) | 8 段模板把 JTBD 与 ODI/Kano 整合一段 |
| Ch 5 竞品图谱 | 8 段模板无对应章 | PRD 前置物不做竞品图谱 (那是 competitive profile) |
| Ch 6 功能架构 | 段 4 OST 解空间 + 段 5 需求 | 8 段模板解空间与需求分两段，OST 强制 ≥3 候选 |
| Ch 7 视觉证据 | 段 7 evidence-table + 段内嵌入 visual | 8 段模板不单独 visual chapter |
| Ch 8 AI 能力映射 | 8 段模板无 (innovation-direction 专属) | — |
| Ch 9 ODI 矩阵 | 段 2 ODI 直接嵌入 | 8 段模板不另起矩阵章 |
| Ch 10 Roadmap | 段 8 未决问题 + 下一步 (含 owner / 时间窗) | 8 段模板 "下一步" 替代 Roadmap |
| Ch 11 验证实验 | 段 8 内嵌 "靠什么会决" 实验设计 | 8 段模板不另起验证实验章 |
| Ch 12 风险 | 段 3 Cagan 4-risks (前置非后置) | 8 段模板风险评估在解空间前 (PRD 思维) |
| Ch 13 附录 | 段 7 evidence-table 附录 | 同 |

### Section-specific assembly

- **段 1 PR-FAQ 加重 (BLUF 等价)**: 
...
**五个 hard gate 是 hard fail 条件**：若任一未达标 (4-risks 缺一类 / OST <3 候选 / 非目标缺 / 三套指标缺 / TM-11 任一未决问题缺 how_to_resolve) → 报告整体 floor fail. 不为分数注水. 失败状态下 段 1 PR-FAQ 显式标 "PRD 前置物未完备，不可直接进入开发 — 待 [缺口列表] 补齐".
...
**Filename suggestion**: `pr-faq-{subject-slug}.md` 或 `prd-input-{subject-slug}.md` (区别于 13 章模板的 `competitive-report-*.md` / `product-capability-report-*.md` / `innovation-direction-report-*.md`).
