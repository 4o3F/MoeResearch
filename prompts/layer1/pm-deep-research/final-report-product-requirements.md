...
Run this checklist over `aspect_reports` + `evidence_index` + `failed_aspects`. For each gap, either (a) trigger one orchestration backfill round — re-call `aspect_research` for the deficient aspect, passing `shared_context.prior_sources` = already-collected evidence (Standard ≤1 round, Deep ≤2 rounds) — or (b) mark explicitly in 段8 (open-questions) and lower the affected confidence. **The five hard gates** (段3 4-risks / 段4 ≥3 候选 / 段5 非目标 / 段6 三套指标 / 段8 TM-11) **may not be silently soft-papered** — they trigger mandatory backfill or explicit "未完备 / 不可推荐" labeling in 段1 PR-FAQ.
...
| 13-section narrative report 13 章 | 8-section PR-FAQ template 8 段 | 区别 |
|---|---|---|
| Ch 1 研究结论摘要 (BLUF/SCQA) | **段 1 PR-FAQ 本身即是 BLUF** | 8-section PR-FAQ template 不另起 BLUF |
| Ch 4 用户人群与 JTBD | 段 2 机会验证 (含 JTBD) | 8-section PR-FAQ template 把 JTBD 与 ODI/Kano 整合一段 |
| Ch 5 竞品图谱 | 8-section PR-FAQ template 无对应章 | PRD 前置物不做竞品图谱 (那是 competitive) |
| Ch 6 功能架构 | 段 4 OST 解空间 + 段 5 需求 | 8-section PR-FAQ template 解空间与需求分两段，OST 强制 ≥3 候选 |
| Ch 7 视觉证据 | 段 7 evidence-table + 段内嵌入 visual | 8-section PR-FAQ template 不单独 visual chapter |
| Ch 8 AI 能力映射 | 8-section PR-FAQ template 无 (innovation-direction 专属) | — |
| Ch 9 ODI 矩阵 | 段 2 ODI 直接嵌入 | 8-section PR-FAQ template 不另起矩阵章 |
| Ch 10 Roadmap | 段 8 未决问题 + 下一步 (含 owner / 时间窗) | 8-section PR-FAQ template "下一步" 替代 Roadmap |
| Ch 11 验证实验 | 段 8 内嵌 "靠什么会决" 实验设计 | 8-section PR-FAQ template 不另起验证实验章 |
| Ch 12 风险 | 段 3 Cagan 4-risks (前置非后置) | 8-section PR-FAQ template 风险评估在解空间前 (PRD 思维) |
| Ch 13 附录 | 段 7 evidence-table 附录 | 同 |

### Section-specific assembly

- **段 1 PR-FAQ 加重 (8-section PR-FAQ template BLUF 等价)**:
...
**Filename suggestion**: `pr-faq-{subject-slug}.md` 或 `prd-input-{subject-slug}.md` (区别于 13-section narrative report 的 `competitive-report-*.md` / `product-capability-report-*.md` / `innovation-direction-report-*.md`).
