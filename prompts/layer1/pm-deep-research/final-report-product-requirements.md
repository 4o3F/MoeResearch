...
| **Cagan 4-risks 覆盖 (hard gate)** | `cagan-four-risks` 缺失/失败，或某类未完备 | **强制 backfill**；仍缺 → 段1 标 "未完备" + 段8 列为开放问题 |
...
| 3 | **Cagan 四大风险** (hard gate) — body: summary; detail → A.3 | **加重** | strategist | `cagan-four-risks` 覆盖四类；full table → Annex A.3 |
| 4 | **Torres OST 解空间** (hard gate) | **加重** | EA + strategist | `ost-solution-space` output |
| 5 | **需求 (功能 / 非功能 / 非目标)** (hard gate) | **加重** | EA + strategist | `requirements-and-metrics` output + trace 回段2 outcome ids |
| 6 | **成功度量** (hard gate) | **加重** | strategist + EA | `requirements-and-metrics` 包含主/次/护栏指标 |
...
**do_not_drop**: 段 1 / 2 / 3 / 4 / 5 / 6 / 8 必出. 段 7 evidence **表必出**（作为 appendix：段后 evidence 表 + 段尾 evidence_refs），但其内容**默认由 Phase B 跨段聚合产出**（不依赖单独的段7 aspect；MoeResearch `evidence_refs` 不许 cite prior_sources by id，单独 spin meta-aggregation aspect 容易制造 provenance mismatch）。即"证据表必出、不单独执行段7 aspect".
...
- **段 3 → body summary + Annex A.3 (hard gate)**:
  - `cagan-four-risks` 覆盖四类风险；
  - aspect 缺失/失败 → Phase A 强制 backfill；仍缺 → 段1 PR 标 "未完备" + 段8 列为开放问题；
...
- Every load-bearing claim in Deep (with `evidence_pack` optional) must have a `claim_id` and a Claim Ledger row. Standard mode extracts 5-10 key claims.
