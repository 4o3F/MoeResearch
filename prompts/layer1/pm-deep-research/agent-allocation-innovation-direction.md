...
Same two persona prompts as competitive / product-capability (MoeResearch has no persona concept; persona = prompt). Task decomposition conditionally appends the search contract and Run Binding, the WebFetch contract, or both contracts according to the selected tools. Cross-cutting quality gates TM-4 (epistemic tagging) + TM-11 (falsifiability) apply to both; **TM-11 is the recommended-bets aspect's hard gate** under innovation-direction:
...
| `whitespace-and-future-capability` | 3+4 | strategist | 白地/value curve + 候选能力、承载力和 unmet 对位 | canvas ≥1；≥2 候选有 Tier 1/2 依据和 unmet 对位 |
...
| `pre-mortem-and-build-cost` | 6+7 | strategist | ≥3 死因 + 对手迭代节奏的 build-cost 代理 | 每死因有机制/触发；build-cost 区间和 ≥1 changelog 时间线 |
...
One MoeResearch aspect = one persona, 所以 profile §5 标 "EA 看 unmet" 在本 profile 收敛到段2 一个 aspect. 段4 中 "对位" 维度由 `whitespace-and-future-capability` (strategist) 通过 `context.prior_sources` fold-in. **不另起 dedicated EA aspect for 段4 对位** (避免 8→9 aspect 增预算 + 增 wave).

### 段6 pre-mortem 强制三死因 hard rule

`pre-mortem-and-build-cost` 的 `success_criteria` 必须显式列：
...
- `ai-upgrade` (本期 default): 段4 强制 ≥1 AI capability candidate; 段1 强制 ≥1 trend 来自技术成熟度类来源.
...
| `standard` | + `unmet-outcomes`, `whitespace-and-future-capability` (4 total) | 加 unmet + 白地/未来能力 |
| `deep` | + `disruption-defensibility`, `pre-mortem-and-build-cost` (**6 total**) | 加颠覆、pre-mortem 和 build-cost; 段8 TM-11 全 tier 启用 |
...
- **Entity-discovery-heavy** (`trend-scan`, `whitespace-and-future-capability`, `disruption-defensibility`) → semantic-discovery provider (e.g. `exa`).
- **User-evidence-heavy** (`unmet-outcomes`) → synthesis provider that 能 surface 大量 user reviews (e.g. `grok`).
- **Synthesis** (`pre-mortem-and-build-cost`, `recommended-bets`) → synthesis provider (e.g. `grok`).
...
6. **Strategist-heavy invariant**: 8 segments 中 7 个由 strategist 拥有，段2 由 EA 拥有。
7. **段6 + 段8 是 hard floor aspect** — 缺 (3 死因 / falsifiability) → 整段 0 分, 拒绝软化.
8. `pre-mortem-and-build-cost` 可复用 prior evidence，减少独立 search 消耗。
