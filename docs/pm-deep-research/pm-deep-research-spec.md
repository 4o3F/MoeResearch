...
> 上游：ROADMAP §3 、 plan、ADR-0006、ADR-0007（待落）、Track B、Track A。
> 引擎接口：[`orchestration-interface.md`](orchestration-interface.md)；评测：`../../evaluation/rubric.md`。
...
> arxiv 来源须**确认录用状态**（非 "under review"/desk-rejected）才入 Tier 1（参见 `citation-audit.md` 的先验真伪纪律）。
...
1. **citation-grounding + abstention（宁少但真）**：无可靠来源时**弃权 / 标"未找到"**，不编。PM DeepResearch 第一天就自我执行（见 `citation-audit.md` 对自身文档的清洗）。
...
> 这条规范同时是 rubric 的整体可读性 floor：机械堆砌即使各维分够也判不通过。所有 capability profile 的 golden / 参考产出必须示范本规范。
...
不达标→报告标置信度警告或对应结论弃权（§6.4）。**Quality floor 同时是 rubric 的硬下限。**
...
Claude-only 不是失败——方法论增强（MECE-6 / 方法库 / 装配 / 证据完整性 / 模板族）是纯 prompt 能力，不依赖 Rust MCP。具体降级流程见 [`../../skills/pm-deep-research/prompts/layer1/claude-only-degradation.md`](../../skills/pm-deep-research/prompts/layer1/claude-only-degradation.md)。
...
>  **不**新增引擎需求；4 能力跑通后若发现共需的新字段，按接口 §6 统一整理提给 4o3F。
...
- v2.0 竞品研究的验证产物（黄金样例 22/24、Phase 3 M4 实跑 22/24）→ 通用规格的方法论承载来源。
- 旧 [`pm-deep-research-competitive-research-spec.md`](pm-deep-research-competitive-research-spec.md) 在 G2-a 完成后转为 redirect 短文件（一段话指向本 spec + `capabilities/competitive.md`）。
- Phase 0 的 3 份 AI 草稿已在 Phase 2 归档至 `../archive/`（保留可追溯，不再作为事实源）。
