...
> Status: ✅ **v2.1 端到端已验证**（黄金 [Runna 训练计划自适应能力 23/24, 2026-05-30](../evaluation/golden/product-capability-runna-training-plan.md)；[R4-c canonical 评分](../evaluation/golden/product-capability-rubric-score.md)）。
...
- ✅ **v2.1 M5 端到端实跑通过（2026-05-30，[23/24](../evaluation/golden/product-capability-rubric-score.md)，超 v2.0 M4 锚点 22/24 +1）**。黄金课题：Runna 训练计划自适应能力深度（海外运动 domain，与 v2.0 Strava AI 升级 narrative 续）。
- **装配契约全实测可行**：6 段 EA-heavy（4:2 EA:Strategist）+ 段5 strategist persona ownership note（EA 数据通过 `prior_sources` 注入）+ family A 13 章 Ch6/7/4 加重 + Ch5 裁为 benchmark 段 + §3.2 capability-specific floor 全过。
- **不需 EA-deep 变体**（v2.1 实测决策成立）—— 通用 [`persona-experience-analyst.md`](../../../prompts/layer2/pm-deep-research/persona-experience-analyst.md) 在体验路径 first-class + 单产品纵深上产出充分。
...
- **关键 M5 发现**：
  - A4 来源质量 1→2（vs M4）—— 装配契约更好引导 strategist 拉多源，不是预算翻倍。
  - C1 = 1 同 M4（4 < 5 visual evidence）—— Skill 层 Layer-2 录屏待办，与 profile 装配无关。
  - 引擎本地补丁延伸：`MAX_SSE_DATA_BYTES 64 KiB → 256 KiB`（CPA gpt-5.5 reasoning chunk 撞顶；需上报 4o3F 上游）。
