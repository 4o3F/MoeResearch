...
| 8 | 综合 + 报告装配 | **Skill** | aspect_reports + 证据 → 按 profile 的 `report_template`（13-section narrative report 13 章 / 8-section PR-FAQ template 8 段 PR-FAQ）+ 机会矩阵 / 解空间 / Roadmap |
| 9 | 自评 quality floor | **Skill** | 报告 → 通用 §9.2 + profile §3.2 + rubric §6 实例化；不达标标警告或弃权 |
...
| Deep | 5–6 | **4**（recommended config；原 8 过搜 execution abort，详 §5.1）| 2–3 | 600000 |
...
### 5.1 上游 search tuning 字段（`e04398d5` / [#7](https://github.com/4o3F/Lapis/issues/7)， 回补）
...
"search_policy": {
  ...
  "freshness": null,
  // ↓ recommended config（v2.0 已启用）：只挑「不增搜索次数」的安全字段
  "recency": "fresh",
  // depth=high_recall ❌ 过搜 execution abort；content_level=detailed ❌ provenance 改写；category ❌ exact-match 不能全局
...
- **现状（2026-06-02，4 能力全部走完）**：
  ...
  - **v2.2 innovation-direction ✅ canonical（cap=6 必须）**（2026-06-02）`recency=fresh` + `max_results=5` + **per-aspect cap=6**（不是 v2.0=4 / v2.1=3）；recommended-bets 综合下注 aspect 在 recency=fresh prompt-hint 下 search appetite ~6，cap=5 持续 execution abort。锚点 24 持平、+11 ev。
  - **v2.3 product-requirements ⛔ NOT canonical**（2026-06-02，honest abstention）；requirements-fn-nfn-nongoals 综合 FR/NFR aspect 在 recency=fresh 下结构性 synthesis-time fragile（4 次 retry 模式齐：cap=8 search execution abort / cap=9 runtime / cap=9+aspect_timeout=900s 撞 execution_policy=600s 复校 / cap=9+双侧 900s 撞 CPA SSE flake）；8-section PR-FAQ template PR-FAQ 合成强度比 v2.2 recommended-bets 高一档。rerun9 anchor 24/24 沿用。等引擎支持 per-aspect `search_policy` override 或 per-aspect `execution_policy.timeout_ms` 后回头。
...
**引擎不是我们做的**——Lapis 由上游 **4o3F** 维护，PM DeepResearch 是消费方（见 ADR-0002）。因此：

- **v2.0 第一版不碰引擎**：纯 prompt+Skill 承载产品字段（§3），用 rubric + 黄金样例实测是否够稳。
...
4o3F 在 2026-05-30 合入两项与 PM DeepResearch 直接相关的修复 / 能力（本地 vendored 副本仍卡在 `02ec7d9` @ 2026-05-28，按 ROADMAP §3 Phase 3  R4 同步延后到 Phase 4 一键安装窗口）：
...
> - **`depth=high_recall` ❌ 危险勿用**：其 `prompt_hint` 怂恿 agent 多搜，撞引擎 search-budget **execution abort**（`agent_loop.rs` 超预算秒杀整个 aspect）；rerun1 cap=8+high_recall **仅 1/6 收敛**——2 个 aspect 被 search-budget 秒死（capability/build-cost），另 2 个 provenance 改写、1 个 network。
> - **`content_level=detailed` ⚠️ 不全局用**：与 `mutated_evidence_provenance` 失败相关。
> - **`category` 不能全局**（exact-match）→ per-aspect prompt-guided，延后 。
> - **铁律**：execution abort 引擎上只挑「不增搜索次数」的字段（`recency`/`max_results` 安全）。
...
## 待办
- [ ]  黄金样例实跑时，验证 §3 的 prompt 承载方案是否够稳（决定 §6 是否触发）。
