# Layer1 ↔ Lapis 编排接口（通用 + capability 路由）

> 目的：把通用规格 [`pm-deep-research-spec.md`](pm-deep-research-spec.md) + 各能力 profile（[`capabilities/`](capabilities/)）的工作流落到 **Lapis 真实 MCP 接口**（依据 [`../mcp-usage.md`](../mcp-usage.md)），明确每步谁做、传什么、schema 缺口怎么补。
> **泛化变更点**：①§1 加 capability 路由步（Step 1.5）；②§2 从"五维→aspect_tasks"泛化为"**profile skeleton → aspect_tasks**"（competitive 为实例）；③§4 4-tier 后处理、§5 预算、§6 引擎边界、§7 降级**对所有 capability 通用**，不变。**Lapis MCP 边界绝不变动**（仍 `aspect_research` + `deep_research`）。
...
| 8 | 综合 + 报告装配 | **Skill** | aspect_reports + 证据 → 按 profile 的 `report_template`（13-section narrative report / 8-section PR-FAQ template）+ 机会矩阵 / 解空间 / Roadmap |
...
| Deep | 5–6 | **4**（recommended config；原 8 触发引擎 search-budget 中止，详 §5.1）| 2–3 | 600000 |

> **完整 budget 形状**（生成 `DeepResearchRequest` 必须给齐，照 `prompts/layer1/task-decomposition.md` 输出 schema）：顶层 `budget{ max_agents, max_concurrent_agents, max_total_model_calls, max_total_search_calls, total_timeout_ms, max_tokens }` + 每 aspect `budget{ max_turns, max_tool_calls, max_search_calls, timeout_ms }`。上表只列了 tier 间差异最大的几项，其余字段也必须填。
...
### 5.1 上游 search tuning 字段（`e04398d5` / [#7](https://github.com/4o3F/Lapis/issues/7)）
...
> **⚠️ 机制说明（引擎 `9db7464` 源码）**：这 4 个字段**只能挂全局 `search_policy`**——`crates/lapis-workflow/src/workflow.rs::aspect_requests` 把同一个 `search_policy` clone 进**每个** aspect；`AspectSpec`（`research.rs`）只有 `search_provider`，**没有 per-aspect search 字段**。要 per-aspect 差异化只能靠 `aspect_agent_prompt` 指示 agent 在调 `search` 工具时自带 `SearchToolArgs`（运行时、非确定性），属未来工作。policy 语义（`policy.rs`）：`depth`/`content_level`/`recency` 是 **ceiling + default**（agent 可往低 rank 调，不能超）；`category` 是 **exact-match**（与 policy 不同即报错）→ **`category` 不能全局**（会逼所有 aspect 同一类）。

**已验证推荐配置**：`recency="fresh"` + `max_results_per_query=5`（已写进 [task-decomposition.md](../../prompts/layer1/pm-deep-research/task-decomposition.md) search_policy）。**不用 `depth=high_recall`**（其 prompt-hint 怂恿过搜，撞引擎 search-budget 执行中止——`agent_loop.rs` 对超预算的 search 是秒杀整个 aspect、无优雅合成回退；`max_search_calls=8`+high_recall 测试仅 1/6 aspect 收敛——其中 2 个 aspect 被 search-budget 秒死，另 2 个 `mutated_evidence_provenance`、1 个 network）；**不全局用 `content_level=detailed`**（与 `mutated_evidence_provenance` 失败相关）。**铁律**：在强制中止引擎上 search-tuning **只挑「不增搜索次数」的字段**（`recency`/`max_results` 安全；`depth=high_recall` 危险）。
...
| `recency` | `default` / `live` / `fresh` / `recent` / `cached` | `default` | 时间新鲜度偏好。`fresh` / `recent` = changelog / 版本时间线 / 趋势扫 / 新闻类 aspect（适合 build-cost 类 aspect 补充来源数量）；`cached` = 容忍旧快照换速度 |
...
"search_policy": {
  ...
  "freshness": null,
  // ↓ 推荐配置：只挑「不增搜索次数」的安全字段
  "recency": "fresh",
  // depth=high_recall ❌ 过搜触发执行中止；content_level=detailed ❌ provenance 改写；category ❌ exact-match 不能全局
...
- **4 能力已验证配置**：
  - **competitive**：`recency=fresh` + `max_results=5` + per-aspect `max_search_calls=4`（validated against golden report）
  - **product-capability**：`recency=fresh` + `max_results=5` + per-aspect `max_search_calls=3`
  - **innovation-direction**：`recency=fresh` + `max_results=5` + **per-aspect `max_search_calls=6`**（recommended-bets 综合 aspect 在 recency=fresh 下 search appetite ~6，`max_search_calls=5` 持续触发执行中止）
  - **product-requirements** ⛔：`recency=fresh` **不推荐**——requirements-fn-nfn-nongoals 综合 FR/NFR aspect 在 recency=fresh 下结构性 synthesis-time fragile（多次 retry 均失败：search-budget 中止 / runtime 超限 / 双侧 timeout 仍触发 execution_policy 复校）；8-section PR-FAQ template 综合 aspect 合成强度高于其它能力。等引擎支持 per-aspect `search_policy` override 或 per-aspect `execution_policy.timeout_ms` 后回头。
...
| `recency=fresh` / `recent` | changelog / 版本时间线 / 趋势扫 / 新闻类 aspect | competitive `build-cost-version-history`；product-capability `benchmark-buildcost-upgrade`；innovation-direction 段 1 趋势扫描；任何看 release notes 的 aspect | 命中率↑可在同 `max_search_calls` 下增加 build-cost 类来源数量（validated: build-cost aspect 经 `recency=fresh` 路径产 4 条 dated official 版本史证据）|
...
| `depth=high_recall` ⚠️ | ~~广度优先扫盲~~ **实测危险：勿用** | — | ~~B1 skeleton 覆盖~~ |

> **⚠️ 重要说明（引擎 `9db7464` 源码验证）**：
...
> - **`recency=fresh` ✅ 已验证**：competitive build-cost aspect 经 fresh 路径产 4 条 dated official 版本史证据（相比默认 2 条）。
> - **`depth=high_recall` ❌ 危险勿用**：其 `prompt_hint` 怂恿 agent 多搜，撞引擎 search-budget 执行中止（`agent_loop.rs` 超预算秒杀整个 aspect）；`max_search_calls=8`+high_recall 测试仅 1/6 aspect 收敛——2 个 aspect 被秒死（capability/build-cost），另 2 个 provenance 改写、1 个 network。
> - **`content_level=detailed` ⚠️ 不全局用**：与 `mutated_evidence_provenance` 失败相关。
> - **`category` 不能全局**（exact-match）→ per-aspect prompt-guided，为后续工作。
> - **铁律**：引擎对 search-budget 超限强制中止；只挑「不增搜索次数」的字段（`recency`/`max_results` 安全）。
> - **4 能力配置要点**：competitive(`max_search_calls=4`) / product-capability(`max_search_calls=3`) / innovation-direction(**`max_search_calls=6` 必须**——综合 aspect search appetite ~6) / product-requirements(**不推荐** recency=fresh——requirements-fn-nfn-nongoals synthesis-time fragile）。**关键**：recency=fresh 的 prompt-hint 把综合 aspect 的 search appetite **推高 1-2 次**——每能力的 max_search_calls 因综合 aspect 强度而异，**不能一刀切**。
