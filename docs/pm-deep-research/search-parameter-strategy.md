...
> Status: 设计草稿（2026-05-29），**核心提案已于上游 #7 / `e04398d5` 落地**（`SearchPolicy` 新增 `depth`/`content_level`/`recency`/`category` 4 个 provider-neutral Option 字段）。本文保留为设计/需求源文档，记录上游引擎扩展的动机与取舍。
> 目的：回答"Exa 这类搜索参数（`type`/`category`/内容深度/`maxAgeHours` …）应该写进配置文件、还是由接口/运行时决定"。本文给开发者讲清**业务逻辑**、**官方 Exa MCP 的取舍**、以及**我们从需求出发的建议**。
> 引擎边界：搜索引擎（`lapis-search`）由上游 **4o3F** 维护，PM DeepResearch 是消费方（PM DR 不修引擎，需求走 issue/PR）。因此本文是**提给 4o3F 的接口扩展需求**，不含 PM DeepResearch 侧代码改动——与 [`orchestration-interface.md §6`](orchestration-interface.md) 同一模式。
...
- 相关 spec：[`orchestration-interface.md`](orchestration-interface.md)、[`../configuration.md`](../configuration.md)、[`../mcp-usage.md`](../mcp-usage.md)
