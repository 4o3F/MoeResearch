...
pub struct OnboardArgs {
    ...
    #[arg(long)]
    pub yes: bool,
    /// Register MoeResearch with Claude Code after config setup.
    #[arg(long)]
...
fn run_dry_run(
    ...
    let generated_env_names = if config_path.exists() && !args.force {
        ...
        tracing::info!(config = %config_path.display(), "would use existing MoeResearch config");
        None
    } else {
...
fn ensure_config(args: &OnboardArgs, config_path: &Path, has_provider_flags: bool) -> Result<()> {
    if config_path.exists() {
        if args.force {
            ...
            ensure_existing_config_allows_flags(config_path, has_provider_flags)?;
            tracing::info!(config = %config_path.display(), "using existing MoeResearch config");
        }
    } else {
...
fn register_args(args: &OnboardArgs, config_path: PathBuf, dry_run: bool) -> McpRegisterArgs {
    McpRegisterArgs {
        ...
        name: "moeresearch".to_owned(),
        scope: args.scope,
        dry_run,
        yes: args.yes,
        claude_bin: PathBuf::from("claude"),
        moe_research_bin: None,
    }
}
...
fn log_register_next_step(config_path: &std::path::Path, scope: McpScope) {
    tracing::info!(
        ...
        "next: moeresearch mcp register --scope <scope> --config <path>"
    );
}
...
fn log_generated_mcp_dry_run(args: &McpRegisterArgs, env_names: Vec<String>) -> Result<()> {
    ...
    let moe_research_bin = std::env::current_exe().map_err(|source| Error::Internal {
    ...
    tracing::info!(
        ...
        config = %mcp_servers_json(&args.name, &moe_research_bin, &config_path, &env_vars),
...
fn ensure_existing_config_allows_flags(config_path: &Path, has_provider_flags: bool) -> Result<()> {
    if has_provider_flags {
        return Err(Error::InvalidInput {
            message: format!(
                "{} already exists; provider flags only apply when generating a config. Edit {} or rerun with `moeresearch onboard --force --config {}`",
                config_path.display(),
                config_path.display(),
                config_path.display()
            ),
