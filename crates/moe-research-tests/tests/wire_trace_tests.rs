...
//! `moe_research_net::reqwest_client`.
//!
        ...
        tokio::spawn(async move {
            loop {
                ...
                tokio::spawn(async move {
                    ...
                    //    succeeds on text bodies because moeresearch only parses
...
async fn wire_trace_disabled_emits_no_body_events() {
    ...
    let (buffer, _guard) = capture_tracing("moe_research_net=debug").await;
...
async fn response_body_read_failure_is_retryable() {
    ...
    let (buffer, _guard) = capture_tracing("moe_research_net::reqwest_client=debug").await;
    let client = build_client_with_retries(1);
    let response = client
        .send_json(request_to(server.url("/responses")))
        .await
