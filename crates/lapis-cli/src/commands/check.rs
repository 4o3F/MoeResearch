...
pub fn run(args: CheckArgs) -> Result<()> {
    ...
    if rows.iter().any(|row| row.status == CheckStatus::Fail) {
        return Err(Error::InvalidInput {
            message: "lapis check failed".to_owned(),
        });
    }
    Ok(())
}
...
fn check_provider_environment(config: &LapisConfig) -> Vec<CheckRow> {
    let mut rows = Vec::new();
    ...
    if config.model.enabled_count() == 0 {
        rows.push(CheckRow::fail(
            ...
            Some("enable [model.providers.openai] in lapis.toml".to_owned()),
            Some("docs/configuration.md".to_owned()),
        ));
    }
...
fn check_live_probe_support(config: &LapisConfig) -> Vec<CheckRow> {
    config
...
fn check_mcp_smoke(config_path: &Path) -> CheckRow {
        ...
        .spawn()
    {
        ...
        Err(source) => {
            return CheckRow::fail(
                ...
                Some("run `lapis serve --config <path>` for details".to_owned()),
                None,
            );
        }
    };
    ...
    if let Some(mut stdin) = child.stdin.take() {
        let messages = concat!(
            "{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"initialize\",\"params\":{\"protocolVersion\":\"2025-11-25\",\"capabilities\":{},\"clientInfo\":{\"name\":\"lapis-check\",\"version\":\"0.1.0\"}}}\n",
    ...
    if tools_list_contains(&stdout, &["aspect_research", "deep_research"]) {
        ...
        CheckRow::fail(
            "mcp",
            "tools/list did not expose expected Lapis tools",
            Some("run `lapis serve --config <path>` and inspect stderr".to_owned()),
...
fn log_check_rows(rows: &[CheckRow], json: bool, verbose: bool) {
    if json {
        ...
        tracing::info!(report = %serde_json::to_string_pretty(&rows).expect("check rows must serialize"), "lapis check report");
        return;
    }

    tracing::info!("lapis check");
    for row in rows {
        tracing::info!(
